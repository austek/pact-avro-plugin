...
use pact_protobuf_plugin::server::AvroPactPlugin;
...
// Rocket server used to pass the Avro messages from/to the verifier. Each request should
...
async fn messages(request: MessageRequest) -> (Status, (ContentType, Vec<u8>)) {
  ...
  if let Some(content_type) = request.contents.message_content_type() {
    if content_type.sub_type == "avro" {
      if let Some(message_type) = content_type.attributes.get("message") {
        match message_type.as_str() {
          "InitPluginRequest" => init_plugin_request(&request).await,
          _ => (Status::BadRequest, (ContentType::Text, Vec::from("Unknown avro message type provided")))
        }
      } else {
        (Status::BadRequest, (ContentType::Text, Vec::from("Not a avro message type provided")))
      }
    } else {
      (Status::BadRequest, (ContentType::Text, Vec::from("Not a avro message")))
    }
  } else {
...
// plugin server init_plugin method, but pass in the Avro message we get from the verifier.
        ...
        match plugin.init_plugin(tonic::Request::new(request)).await {
          Ok(response) => {
            ...
            match response.get_ref().encode(&mut buffer) {
              Ok(_) => {
                (Status::Ok, (ContentType::new("application", "avro").with_params(("message", "InitPluginResponse")),
                              buffer.to_vec()))
              }
              Err(err) => {
...
// Pact verification test. This first starts up a Rocket server that can provide the Avro
...
async fn verify_plugin() {
  ...
  let mut version = "pact-avro-plugin:".to_string();
  ...
  let publish_options = if env::var("CI").map(|v| v == "true").unwrap_or(false)  {
    Some(PublishOptions {
      ...
      build_url: None,
      provider_tags: vec!["pact-avro-plugin".to_string()],
      provider_branch: None
    })
  } else {
    None
