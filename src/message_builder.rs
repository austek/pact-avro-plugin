//! Builder for creating avro messages based on a descriptor
...
/// Builder struct for a Avro message
#[derive(Clone, Debug, PartialEq)]
pub struct MessageBuilder {
  /// Avro descriptor for the file the message belongs to
  pub file_descriptor: FileDescriptorProto,
  /// Avro descriptor for the message
...
impl MessageBuilder {
  ...
  /// Encodes the Avro message into a bytes buffer
      ...
      if let Some(tag) = field_data.descriptor.number {
        match field_data.proto_type {
          ...
          Type::Message => match &value.rtype {
            ...
            RType::Bytes(b) => {
              // Encode a google.avro.BytesValue
              trace!("Encoding a Avro BytesValue");
          ...
          _ => return Err(anyhow!("Avro type {:?} is not supported", field_data.proto_type))
        }
      }
    }
    Ok(())
  ...
  fn encode_enum_value(
    ...
    let enum_value = enum_proto.value.iter().find(|enum_val| enum_val.number == Some(*enum_value))
      .ok_or_else(|| anyhow!("Did not find the enum value {} for the enum {} in the Avro descriptor", enum_value, enum_type_name))?;
  ...
  fn encode_map_field(&self, buffer: &mut BytesMut, field_value: &FieldValueInner) -> anyhow::Result<()> {
    ...
    if !field_value.values.is_empty() {
      ...
      let entry_proto = self.descriptor.nested_type.iter().find(|nested_type| nested_type.name.clone().unwrap_or_default() == entry_name)
        .ok_or_else(|| anyhow!("Did not find the nested type {} for the map field {} in the Avro descriptor", entry_name, entry_type_name))?;

      let key_proto = entry_proto.field.iter().find(|f| f.name.clone().unwrap_or_default() == "key")
        .ok_or_else(|| anyhow!("Did not find the field descriptor for the key for the map field {} in the Avro descriptor", entry_type_name))?;
      let value_proto = entry_proto.field.iter().find(|f| f.name.clone().unwrap_or_default() == "value")
        .ok_or_else(|| anyhow!("Did not find the field descriptor for the value for the map field {} in the Avro descriptor", entry_type_name))?;
...
/// Rust type to use for a avro type
#[derive(Clone, Debug, PartialEq)]
pub enum RType {
  /// String value
  String(String),
...
mod tests {
  ...
  fn encode_message_bytes_test() {
    ...
    //   google.avro.BytesValue content = 2;
    ...
    let field2 = message_field_descriptor!("content", 2, ".google.avro.BytesValue");
    ...
    expect!(message.generate_markup("")).to(be_ok().value(
      "|```avro
         |message Body {
         |    string contentType = 1;
         |    message .google.avro.BytesValue content = 2;
  ...
  fn encode_message_with_map_field_test() {
    ...
    let values_field_descriptor = message_field_descriptor!("values", 2, ".google.avro.Struct");
    ...
    let struct_fields_descriptor = FieldDescriptorProto {
      ...
      label: Some(Repeated as i32),
      r#type: Some(field_descriptor_proto::Type::Message as i32),
      type_name: Some(".google.avro.Struct.FieldsEntry".to_string()),
      extendee: None,
      default_value: None,
      oneof_index: None,
    ...
    let value_descriptor = FieldDescriptorProto {
      ...
      r#type: Some(field_descriptor_proto::Type::Message as i32),
      type_name: Some(".google.avro.Value".to_string()),
      extendee: None,
      default_value: None,
      oneof_index: None,
    ...
    let actual_data = actual_map_values.iter().map(|f| {
      ...
      match &f.data {
        AvroFieldData::Message(d, _) => d.clone(),
  ...
  //   //   google.avro.Struct messageMetadata = 4;
