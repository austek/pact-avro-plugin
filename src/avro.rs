//! Module for processing and comparing avro messages
...
/// Process the provided avro file and configure the interaction
pub(crate) async fn process_proto(
  ...
  if let Some(message_type) = config.get("pact:message-type") {
    ...
    debug!("Configuring a Avro message {}", message);
    let result = configure_avro_message(message.as_str(), config, descriptor,
    ...
    debug!("Configuring a Avro service {}", service_name);
    let (request_part, response_part) = configure_avro_service(service_name.as_str(), config, descriptor,
...
/// Configure the interaction for a Avro service method, which has an input and output message
fn configure_avro_service(
  service_name: &str,
  config: &BTreeMap<String, prost_types::Value>,
  ...
  trace!(">> configure_avro_service({service_name}, {config:?}, {descriptor_hash})");
  ...
  construct_avro_interaction_for_service(service_descriptor, config, service,
...
/// Constructs an interaction for the given Avro service descriptor
fn construct_avro_interaction_for_service(
  ...
  trace!(">> construct_avro_interaction_for_service({config:?}, {service_name}, {method_name})");
  ...
  let request_part = if request_part_config.is_empty() {
    ...
    let interaction = construct_avro_interaction_for_message(&request_descriptor,
  ...
  for config in response_part_config {
    let interaction = construct_avro_interaction_for_message(
...
/// Configure the interaction for a single Avro message
fn configure_avro_message(
  message_name: &str,
  config: &BTreeMap<String, prost_types::Value>,
  descriptor: &FileDescriptorProto,
  descriptor_hash: &str,
  ...
  trace!(">> configure_avro_message({}, {:?})", message_name, descriptor_hash);
  ...
  construct_avro_interaction_for_message(message_descriptor, config, message_name, "", descriptor, all_descriptors)
    .map(|interaction| {
      InteractionResponse {
...
/// Constructs an interaction for the given Avro message descriptor
#[instrument(ret, skip(message_descriptor, file_descriptor, all_descriptors))]
fn construct_avro_interaction_for_message(
  ...
  trace!(">> construct_avro_interaction_for_message({}, {}, {:?}, {:?})", message_name,
  ...
  debug!("Building message {} from Avro descriptor", message_name);
  ...
  Ok(InteractionResponse {
    contents: Some(Body {
      content_type: format!("application/avro;message={}", message_name),
...
fn build_single_embedded_field_value(
  ...
  match type_name.as_str() {
    ".google.avro.BytesValue" => {
      debug!("Field is a Avro BytesValue");
      if let Value::String(_) = value {
        build_field_value(path, message_builder, field_type, field_descriptor, field,
                          value, matching_rules, generators, all_descriptors)
      } else {
        Err(anyhow!("Fields of type google.avro.BytesValue must be configured with a single string value"))
      }
    }
    ".google.avro.Struct" => {
      debug!("Field is a Avro Struct");
...
/// Create a field value of type google.avro.Struct
fn build_struct_field(
  ...
  match field_value {
    ...
    _ => Err(anyhow!("google.avro.Struct fields need to be configured with a Map, got {:?}", field_value))
  }
}
  ...
  match value {
    ...
    Value::Array(a) => {
      ...
      if let Some(err) = values.find_map(|v| v.err()) {
        return Err(anyhow!("Could not construct a Avro list value - {}", err))
      }
...
fn value_for_type(
  ...
  match t {
    ...
    Type::Message => {
      if type_name == ".google.avro.BytesValue" {
        Ok(MessageFieldValue::bytes(field_name, field_value))
      } else {
        Err(anyhow!("value_for_type: Avro field {} has an unsupported type {:?} {}", field_name, t, type_name))
      }
    }
    ...
    Type::Enum => {
      ...
      if let Some((n, desc)) = result {
        ...
        Err(anyhow!("Avro enum value {} has no value {}", type_name, field_value))
      }
    }
    _ => Err(anyhow!("Avro field {} has an unsupported type {:?}", field_name, t))
  }
}
...
pub(crate) mod tests {
  ...
  use crate::avro::{
    ...
    construct_avro_interaction_for_message,
    construct_avro_interaction_for_service,
    value_for_type
  };
  ...
  #[test]
  fn construct_avro_interaction_for_message_test() {
    ...
    let result = construct_avro_interaction_for_message(&message_descriptor, &config, "test_message", "", &file_descriptor, &hashmap!{}).unwrap();

    let body = result.contents.as_ref().unwrap();
    expect!(body.content_type.as_str()).to(be_equal_to("application/avro;message=test_message"));
  ...
  #[test]
  fn construct_avro_interaction_for_service_returns_error_on_invalid_request_type() {
    ...
    let service_descriptor = ServiceDescriptorProto {
      ...
      method: vec![
        MethodDescriptorProto {
          ...
          input_type: Some(".google.avro.StringValue".to_string()),
    ...
    let result = construct_avro_interaction_for_service(&service_descriptor, &config,
  ...
  #[test_log::test]
  fn construct_avro_interaction_for_service_supports_string_value_type() {
    let string_descriptor = DescriptorProto {
      name: Some("StringValue".to_string()),
      field: vec![
        FieldDescriptorProto {
