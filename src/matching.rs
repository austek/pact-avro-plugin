//! Functions for matching Avro messages
...
/// Match a Avro service call, which has an input and output message
pub fn match_service(
  ...
  }).ok_or_else(|| anyhow!("Did not find the method {} in the Avro file descriptor for service '{}'", method_name, service_name))?;
...
pub(crate) fn compare(
  ...
  expected_message: &[AvroField],
  actual_message: &[AvroField],
...
fn compare_message(
  path: DocPath,
  expected_message_fields: &[AvroField],
  actual_message_fields: &[AvroField],
  matching_context: &dyn MatchingContext,
  message_descriptor: &DescriptorProto,
  descriptors: &FileDescriptorSet,
) -> anyhow::Result<BodyMatchResult> {
...
fn compare_field(
  ...
  match (&field.data, &actual.data) {
    ...
    (AvroFieldData::Integer32(n1), AvroFieldData::Integer32(n2)) => {
      trace!("Comparing Integer32 values");
      compare_value(path, field, *n1, *n2, n1.to_string().as_str(), n2.to_string().as_str(), matching_context)
    },
    (AvroFieldData::UInteger64(n1), AvroFieldData::UInteger64(n2)) => {
      trace!("Comparing UInteger64 values");
      compare_value(path, field, *n1, *n2, n1.to_string().as_str(), n2.to_string().as_str(), matching_context)
    },
    (AvroFieldData::Integer64(n1), AvroFieldData::Integer64(n2)) => {
      trace!("Comparing Integer64 values");
      compare_value(path, field, *n1, *n2, n1.to_string().as_str(), n2.to_string().as_str(), matching_context)
    },
    (AvroFieldData::Float(n1), AvroFieldData::Float(n2)) => {
    ...
    (AvroFieldData::Double(n1), AvroFieldData::Double(n2)) => {
      trace!("Comparing Double values");
      compare_value(path, field, *n1, *n2, n1.to_string().as_str(), n2.to_string().as_str(), matching_context)
    },
    (AvroFieldData::Bytes(b1), AvroFieldData::Bytes(b2)) => {
    ...
    (AvroFieldData::Enum(b1, descriptor), AvroFieldData::Enum(b2, _)) => {
    ...
    (AvroFieldData::Message(b1, message_descriptor), AvroFieldData::Message(b2, _)) => {
      ...
      match &descriptor.type_name {
        Some(name) => match name.as_str() {
          ...
          ".google.avro.Struct" => {
            debug!("Field is a Avro Struct, will compare it as JSON");
...
fn compare_repeated_field(
  path: &DocPath,
  descriptor: &FieldDescriptorProto,
  expected_fields: &[AvroField],
  actual_fields: &[AvroField],
  matching_context: &dyn MatchingContext,
  descriptors: &FileDescriptorSet
) -> Vec<Mismatch> {
...
fn compare_map_field(
  path: &DocPath,
  descriptor: &FieldDescriptorProto,
  expected_fields: Vec<&AvroField>,
  actual_fields: Vec<&AvroField>,
  matching_context: &dyn MatchingContext,
  descriptors: &FileDescriptorSet
) -> Vec<Mismatch> {
  ...
  if expected_fields.is_empty() && !actual_fields.is_empty() && matching_context.config() == DiffConfig::NoUnexpectedKeys {
    ...
    let expected_map = expected_fields.iter()
      .filter_map(|f| {
        match &f.data {
          AvroFieldData::Message(d, descriptor) => decode_message_map_entry(descriptor, d, descriptors).ok(),
          _ => None
        }
      })
...
/// Struct to represent a avro map entry
#[derive(Clone, Debug)]
struct MapEntry {
...
fn decode_message_map_entry(
  ...
  let key_str = match &key.data {
    AvroFieldData::String(s) => s.clone(),
...
fn find_field_descriptor(field: &AvroField, descriptor: &DescriptorProto) -> Option<FieldDescriptorProto> {
...
mod tests {
  ...
  use crate::message_decoder::AvroField;
  ...
  fn compare_message_where_the_actual_field_is_missing_due_it_being_the_default_enum_value() {
    ...
    let expected = vec![
      AvroField {
        ...
        data: AvroFieldData::Message(vec![8, 0, 18, 4, 116, 101, 115, 116], descriptor.clone())
      }
    ];
    let actual = vec![
        AvroField {
          field_num: 1, 
          wire_type: WireType::LengthDelimited, 
          data: AvroFieldData::Message(vec![18, 8, 112, 114, 111, 116, 111, 98, 117, 102, 26, 54, 10, 13, 99, 111, 110, 116, 101, 110, 116, 45, 116, 121, 112, 101, 115, 18, 37, 97, 112, 112, 108, 105, 99, 97, 116, 105, 111, 110, 47, 112, 114, 111, 116, 111, 98, 117, 102, 59, 97, 112, 112, 108, 105, 99, 97, 116, 105, 111, 110, 47, 103, 114, 112, 99], descriptor.clone())
        }
    ];
