...
/// Convert a Avro Struct to a BTree Map
...
pub fn find_message_field_by_name(descriptor: &DescriptorProto, field_data: Vec<AvroField>, field_name: &str) -> Option<AvroField> {
      ...
      match &value.name {
        Some(name) => {
          object.insert(name.clone(), match &field.data {
            AvroFieldData::String(s) => serde_json::Value::String(s.clone()),
            AvroFieldData::Boolean(b) => serde_json::Value::Bool(*b),
            AvroFieldData::UInteger32(n) => json!(n),
            AvroFieldData::Integer32(n) => json!(n),
            AvroFieldData::UInteger64(n) => json!(n),
            AvroFieldData::Integer64(n) => json!(n),
            AvroFieldData::Float(n) => json!(n),
            AvroFieldData::Double(n) => json!(n),
            AvroFieldData::Bytes(b) => serde_json::Value::Array(b.iter().map(|v| json!(v)).collect()),
            AvroFieldData::Enum(n, descriptor) => serde_json::Value::String(enum_name(*n, descriptor)),
            AvroFieldData::Message(b, descriptor) => {
            ...
            AvroFieldData::Unknown(b) => serde_json::Value::Array(b.iter().map(|v| json!(v)).collect())
          });
        }
...
pub(crate) fn lookup_service_descriptors_for_interaction(
  ...
  let interaction_config = lookup_interaction_config(interaction)
    .ok_or_else(|| anyhow!("Interaction does not have any Avro configuration"))?;
  ...
  let plugin_config = pact.plugin_data.iter()
    .find(|data| data.name == "avro")
    .map(|data| &data.configuration)
    .ok_or_else(|| anyhow!("Did not find any Avro configuration in the Pact file"))?
    .iter()
  ...
  }).ok_or_else(|| anyhow!("Did not find the method {} in the Avro file descriptor for service '{}'", method_name, service))?;
