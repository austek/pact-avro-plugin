...
impl PactPlugin for AvroPactPlugin {
  ...
  async fn start_mock_server(
    ...
    trace!("Got pact {pact:?}");
    // Check for the plugin specific configuration for the Avro descriptors
    let plugin_config = match pact.plugin_data.iter().find(|pd| pd.name == "avro") {
      None => {
        error!("Provided Pact file does not have any Avro descriptors");
        return Ok(tonic::Response::new(proto::StartMockServerResponse {
          response: Some(proto::start_mock_server_response::Response::Error("Provided Pact file does not have any Avro descriptors".to_string())),
          .. proto::StartMockServerResponse::default()
        }))
      }
...
mod tests {
  ...
  fn get_mock_server_results_test() {
    ...
    let (ok, results) = AvroPactPlugin::get_mock_server_results(&mock_results);
  ...
  async fn start_mock_server_returns_an_error_if_the_pact_does_not_have_any_descriptors() {
    ...
    let response = result.unwrap();
    if let Some(start_mock_server_response::Response::Error(message)) = &response.get_ref().response {
      expect!(message).to(be_equal_to("Provided Pact file does not have any Avro descriptors"));
    } else {
      panic!("Was expecting an error message");
    }
  }
