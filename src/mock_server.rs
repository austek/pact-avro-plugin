...
impl GrpcMockServer
{
  ...
  pub async fn start_server(mut self, host_interface: &str, port: u32, tls: bool) -> anyhow::Result<SocketAddr> {
    ...
    if self.descriptors.is_empty() {
      return Err(anyhow!("Pact file does not contain any Avro descriptors"));
    }
    ...
    self.routes = self.pact.interactions.iter()
      ...
      .filter_map(|i| i.plugin_config.get("avro").map(|p| (p.clone(), i.clone())))
      .filter_map(|(c, i)| {
          ...
          if let Some(descriptors) = self.descriptors.get(json_to_string(key).as_str()) {
            if let Some(service) = c.get("service") {
              if let Some((service_name, method_name)) = json_to_string(service).split_once('/') {
                ...
                // avro service was not properly formed <SERViCE>/<METHOD>
                None
              }
            } else {
              // avro plugin configuration section did not have a service defined
              None
            }
          } else {
            // avro plugin configuration section did not have a matching key to the descriptors
            None
          }
        } else {
          // Interaction did not have a avro plugin configuration section
          None
        }
      }).collect();
