...
You can run all tests by executing `sbt test`.

There is a Pact test that verifies the plugin against the Pact file published to a local Pact broker. A docker compose file is provided, run the container
